...
#include <string>
#include <type_traits>
#include <utils/csprng/csprng.h>
#include <utils/csprng/is_iterable.hpp>
...
namespace duthomhas {
    ...
    public:
        ...
        csprng(Iterator begin, Iterator end) :
            internal(csprng_create()), sseq(internal, std::distance(begin, end)) {}

        template<typename T>
        csprng(std::initializer_list<T> xs) : internal(csprng_create()), sseq(internal, xs.size()) {}
        ...
        struct sseq_type {
            ...
            sseq_type(CSPRNG& internal, std::size_t seed_seq_size) :
                internal(internal), seed_seq_size(seed_seq_size) {}
        };
    ...
    public:
        ...
        typename std::enable_if<is_iterable<Iterable>::value, Iterable&>::type operator()(Iterable&& value) {
            for (auto& v : value) {
                operator()(v);
            }
            return value;
