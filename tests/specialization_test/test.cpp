...
namespace ElGamal {
    ...
    private:
        ...
        constexpr void reduce() {
            ...
            F_256 s7(
                {m_digits[12], m_digits[13], m_digits[14], m_digits[15], 0, 0, m_digits[9], m_digits[11]});
            F_256 s8({m_digits[13],
                      ...
                      m_digits[12]});
            F_256 s9(
                {m_digits[14], m_digits[15], 0, m_digits[9], m_digits[10], m_digits[11], 0, m_digits[13]});
        ...
        static constexpr digits max_mod_p = {
            4294967294U, 4294967295U, 4294967295U, 0U, 0U, 0U, 1U, 4294967295U};
        ...
        static constexpr digits p_values = {
            4294967295U, 4294967295U, 4294967295U, 0U, 0U, 0U, 1U, 4294967295U};
    ...
    public:
        ...
        friend constexpr EllipticCurvePoint operator+(EllipticCurvePoint&& lhs,
                                                      const EllipticCurvePoint& rhs) {
            lhs += rhs;
            return lhs;
        }
    ...
    constexpr F_256 generate_random_message() {
        F_256 result;
...
TEST(CorrectnessTest, Encryption) {
    ElGamal::Keys keys = ElGamal::generate_keys();

    for (size_t i = 0; i < c_correctness_test_encryption_n; ++i) {
        ElGamal::F_256 message = ElGamal::generate_random_message();
        ElGamal::EncryptedMessage enc = ElGamal::encrypt(message, keys.public_key);
        ElGamal::F_256 decrypted_message = ElGamal::decrypt(enc, keys.private_key);
